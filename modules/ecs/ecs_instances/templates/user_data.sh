...
aws s3 cp s3://${var.s3_bucket}/cloudformation/awslogs.conf /etc/awslogs/awslogs.conf
sed -i "s/clustername/${var.namespace}-${terraform.workspace}/g" /etc/awslogs/awslogs.conf
sed -i "s/instanceID/`curl -s http://169.254.169.254/latest/meta-data/instance-id`/g" /etc/awslogs/awslogs.conf
service awslogs start
chkconfig awslogs on

echo ECS_CLUSTER=${var.namespace}-${terraform.workspace} >> /etc/ecs/ecs.config
